...
async function fetchProductDetails(productId) {
    try {
        ...
        // Kép URL-ek szűrése, hogy ne legyen benne NULL
        const images = [product.img1, product.img2, product.img3].filter(img => img !== null);

        // Termék konténer kiválasztása
        const productContainer = document.getElementById('produts-cards');
        productContainer.innerHTML = ""; // Előző tartalom törlése

        // Generáljuk a carousel HTML-t
        let carouselHTML = `
            <div class="product-card">
                <div class="carousel-container">
                    <div class="carousel">
                        ${images.map((img, index) => `
                            <img class="carousel-slide ${index === 0 ? 'active' : ''}" src="/uploads/${img}" alt="Product Image">
                        `).join("")}
                    </div>
                </div>
                <h3 class="product-name">${product.brand} | ${product.product_name}</h3> 
                <p class="product-category">${product.category} | ${product.color} | ${product.size}</p>
                <p class="product-price">${product.price}$</p>
                <p class="stock-status">${product.is_in_stock > 0 ? "In Stock" : "Out of Stock"}</p>
                <button class="add-card-btn" onclick="addToCart(event, ${product.product_id})">
                    <p>Add to Cart</p>
                </button>
            </div>
        `;
        ...
        // HTML beillesztése
        productContainer.innerHTML = carouselHTML;

        // Ha több kép van, akkor elindítjuk az automatikus váltást
        if (images.length > 1) {
            startCarousel();
        }
